...
export class Player extends TransformNode {
    ...
    constructor(assets, scene: Scene, shadowGenerator: ShadowGenerator, input?, private _onFallCallback?: () => void) {
        super("player", scene);
        this.scene = scene;
    ...
    private _beforeRenderUpdate(): void {
        ...
        // Mỗi frame, game kiểm tra vị trí Y của nhân vật (this.mesh.position.y)
        // Nếu thấp hơn -10 (tức là đã rơi quá sâu so với mặt đất), sẽ kích hoạt callback onFallCallback()
        // Callback này được truyền từ app.ts khi khởi tạo Player
        if (this.mesh.position.y < -10) {
            this.scene.onBeforeRenderObservable.clear();
            if (this._onFallCallback) {
                this._onFallCallback(); // gọi lose đúng lúc
            }
        }
    ...
    private _setupPlayerCamera() {
        ...
        this.camera.fov = (60 * Math.PI) / 180; // tương đương 30 độ
